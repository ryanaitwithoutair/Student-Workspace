...
const MoodCheckInForm = ({ entry, moodOptions, recordMood, selectedDate }) => {
  const [selectedMood, setSelectedMood] = useState(entry?.mood || 'good');
  const [note, setNote] = useState(entry?.note || '');
  const [hasSaved, setHasSaved] = useState(Boolean(entry));

  const submitCheckIn = (event) => {
    event.preventDefault();
    recordMood(selectedMood, note, selectedDate);
    setHasSaved(true);
  };

  return <form onSubmit={submitCheckIn} className="mt-6"><div className="grid grid-cols-1 gap-2 sm:grid-cols-5">{moodOptions.map((option) => { const active = selectedMood === option.id; return <button key={option.id} type="button" onClick={() => setSelectedMood(option.id)} aria-pressed={active} className={`rounded-2xl border p-3 text-left transition-all focus:outline-none focus:ring-2 focus:ring-emerald-400/50 ${active ? 'border-emerald-400/60 bg-emerald-400/10 shadow-lg shadow-emerald-500/10' : 'border-neutral-800 bg-black/10 hover:border-neutral-600 hover:bg-white/[.035]'}`}><span className="block text-2xl">{option.emoji}</span><span className="mt-2 block text-sm font-bold text-white">{option.label}</span><span className="mt-0.5 block text-[11px] leading-4 text-neutral-400">{option.description}</span></button>; })}</div><label className="mt-5 block text-sm font-semibold text-neutral-200" htmlFor={`mood-note-${selectedDate}`}>A small note <span className="font-normal text-neutral-500">(optional)</span></label><textarea id={`mood-note-${selectedDate}`} value={note} maxLength={240} onChange={(event) => setNote(event.target.value)} placeholder="What’s influencing how you feel?" className="glass-input mt-2 h-28 w-full resize-none rounded-2xl p-3 text-sm" /><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-neutral-500">{note.length}/240 characters</p><button type="submit" className="btn-emerald inline-flex items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold"><CheckCircle2 className="h-4 w-4" />{hasSaved ? 'Update check-in' : 'Save check-in'}</button></div></form>;
};
...
export const MoodView = () => {
  ...
  const week = useMemo(() => Array.from({ length: 7 }, (_, index) => { const date = dateBefore(selectedDate, 6 - index); return { date, entry: moodsByDate.get(date) }; }), [moodsByDate, selectedDate]);
  const weeklyEntries = week.map(({ entry }) => entry).filter(Boolean);
  const weeklyAverage = weeklyEntries.length ? (weeklyEntries.reduce((sum, entry) => sum + (moodOptions.find((option) => option.id === entry.mood)?.score || 0), 0) / weeklyEntries.length).toFixed(1) : null;
